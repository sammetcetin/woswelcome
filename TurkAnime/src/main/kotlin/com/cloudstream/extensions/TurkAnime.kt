...
class TurkAnime : MainAPI() {
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        if (iframe == null || iframe.contains("a-ads.com")) {
            for (button in document.select("button[onclick*='IndexIcerik']")) {
                val buttonLink = button.attr("onclick")
                    .substringAfter("IndexIcerik('")
                    .substringBefore("'")
                    .takeIf { it.isNotBlank() }
                    ?.let { fixUrlNull(it) }
                    ?: continue
                val subDocument = app.get(
                    buttonLink,
                    headers = mapOf("X-Requested-With" to "XMLHttpRequest"),
                ).document
                val directUrl = subDocument.selectFirst("div.artplayer-app[data-url]")
                    ?.attr("data-url")
                ...
                val subFrame = fixUrlNull(subDocument.selectFirst("iframe")?.attr("src")) ?: continue
                iframe2Load(subDocument, subFrame, subtitleCallback, callback)
            }
        } else {
