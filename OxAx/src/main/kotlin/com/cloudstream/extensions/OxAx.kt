...
class OxAx : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val channels = app.get(request.data).parsedSafe<ChannelResponse>()!!.channels.map { channel ->
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        return app.get("${mainUrl}/search/${query}").parsedSafe<ChannelResponse>()!!.channels.map { channel ->
            newLiveSearchResponse(channel.detail.title, "http://oxax.tv/${channel.slug}.html", TvType.Live) {
                this.posterUrl = channel.detail.img
                this.lang = "RU"
            }
