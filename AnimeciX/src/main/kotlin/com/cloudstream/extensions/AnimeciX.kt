...
class AnimeciX : MainAPI() {
    ...
    private val healthProbeUrl = "$mainUrl/secure/titles?type=series&onlyStreamable=true&page=1&perPage=16"
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        if (response.title.titleType == "anime") {
            for (sezon in response.title.seasons) {
                val sezonResponse = app.get("${mainUrl}/secure/related-videos?episode=1&season=${sezon.number}&videoId=0&titleId=${titleId}", headers = apiHeaders).parsedSafe<TitleVideos>() ?: return null
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        Log.d("ACX", "data » $data")
        val iframeLink = app.get("${mainUrl}/${data}", headers = apiHeaders, referer="${mainUrl}/").url
